...
@Slf4j
@Component
public class DwfwContentFilling implements ContentFilling {
    ...
    public String execute(Document document, Range range, Map<String, String> contents, long taskId) throws Exception {
        ...
        StringBuilder qfyjBuilder = new StringBuilder();
        StringBuilder shyjBuilder = new StringBuilder();
        ...
        while(iterator.hasNext()){
            ...
            String commentType = judgeCommentType(wfInfor.getWi71(),tempMaps);

            if("qfyj".equals(commentType)){
                qfyjBuilder.append(objectToHtml(new Comment(commentStr,new DateTime(wfInfor.getWi11()).toString(DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")),getUserName(sysUsersService,wfInfor.getWi05()))));
            }
            if("shyj".equals(commentType)){
                shyjBuilder.append(objectToHtml(new Comment(commentStr,new DateTime(wfInfor.getWi11()).toString(DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")),getUserName(sysUsersService,wfInfor.getWi05()))));
            }

        }

        builder.moveToBookmark("qfyj");
        builder.insertHtml(qfyjBuilder.toString());
        builder.moveToBookmark("shyj");
        builder.insertHtml(shyjBuilder.toString());
        range.replace(Pattern.compile("\\["+"qfyj]"),"",findReplaceOptions);
        range.replace(Pattern.compile("\\["+"shyj]"),"",findReplaceOptions);
    ...
    public Map<String,List<String>> roleMaps(long formId){
        ...
        while(iterator.hasNext()){
            ...
            if("qfyj".equals(ctrlpropertyView.getSi03())){
                roleMaps.put("qfyj", StringUtils.strRoles(ctrlpropertyView.getSp02()));
            }
            if("shyj".equals(ctrlpropertyView.getSi03())){
                roleMaps.put("shyj", StringUtils.strRoles(ctrlpropertyView.getSp02()));
            }
    ...
    public String judgeCommentType(String wi71, Map<String,List<String>> tempMaps){
        List<String> qfyjList = tempMaps.get("qfyj");
        List<String> shyjList = tempMaps.get("shyj");
        if(qfyjList!=null&&qfyjList.contains(wi71)){
            return "qfyj";
        }
        if(shyjList!=null&&shyjList.contains(wi71)){
            return "shyj";
        }
        return "unknown";
    }
